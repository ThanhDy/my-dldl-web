...
export default function BoneBurningPage() {
  const [data, setData] = useState<BoneBurning[]>([]);
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.data.length > 0) {
          setActiveTab(result.data[0].type);
        }
        // Khởi tạo mức mở khóa mặc định là cấp 7 (max) cho mockdata hoặc cấp 1
        const initialUnlocks: Record<string, number> = {};
        result.data.forEach((item: BoneBurning) => {
  ...
  const currentSystem = data.find(s => s.type === activeTab);
      ...
      <div className="max-w-5xl mx-auto px-6 py-20 relative z-10">
        ...
        {loading ? (
          <div className="py-20 flex flex-col items-center justify-center gap-4">
             ...
             <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-600">Đang khởi tạo dữ liệu...</p>
          </div>
        ) : (
